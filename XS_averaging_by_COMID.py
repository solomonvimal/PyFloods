...
file_name = 'XS_COMID_averaged4.csv'
...
for i in range(len(point_xsids)-1):# leave the last point for simplicity
    ...
    if int(point_xsids[i]) == int(point_xsids[i+1]) or point_xsids.index(int(point_xsids[len(point_xsids)-1])) == i+1: ###########
        ...
        d_2 = sum(elevation[0:index_lowest/2])/(float(len(elevation[0:index_lowest/2]))+0.001) - elevation[index_lowest]
        d_4 = sum(elevation[(index_lowest/2):index_lowest])/(float(len(elevation[(index_lowest/2):index_lowest]))+0.001) - elevation[index_lowest]
        d_5 = elevation[index_lowest] - elevation[index_lowest]
        second_half = elevation[index_lowest:len(elevation)]
        l2 = len(second_half)
        try:
            d_6 = sum(second_half[0:l2/2])/(float(l2)/2) - elevation[index_lowest]
            d_8 = sum(second_half[l2/2:])/(float(l2)/2) - elevation[index_lowest]         
        except:
            pass
